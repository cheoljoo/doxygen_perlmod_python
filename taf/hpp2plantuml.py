...
class ComplexEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj,(ClassMethod,ClassVariable,EnumValue,Class,Namespace)):
            # Transform your complex type to a dict or other serializable type
            return obj.__dict__
...
class Container(object):
    ...
    def __init__(self, container_type, name):
        ...
        self._namespace = ''
        self._parent = None
        self._puml_nomember = ''   # no file , no member
        self._puml_nofile = ''     # no file
        self._puml = ''
        self._filename = ''
    ...
    def render(self):
        ...
        container_str = ''
        if self._name in FOCUS_CLASS:
            container_str += self._render_container_def() + ' ' + '#GreenYellow/LightGoldenRodYellow' + ' {\n'
        else:
            container_str += self._render_container_def() + ' ' + ' {\n'
        for member in self._member_list:
            container_str += '\t' + member.render()[:100] + '\n'
        container_str += '}\n'
        self._puml_nofile = container_str
        container_str = ''
        if self._name in FOCUS_CLASS:
            container_str += self._render_container_def() + ' ' + '#GreenYellow/LightGoldenRodYellow' + '\n'
        else:
            container_str += self._render_container_def() + '\n'
        self._puml_nomember = container_str
        container_str = ''
        if self._name in FOCUS_CLASS:
            container_str += self._render_container_def() + ' ' + '#GreenYellow/LightGoldenRodYellow' + ' {\n'
        else:
        ...
        for member in self._member_list:
            container_str += '\t' + member.render()[:100] + '\n'
        if self._namespace.strip():
            container_str += '\t-- namespace --\n'
            container_str += '\t'+self._namespace + '\n'
        container_str += '\t-- filename --\n'
        container_str += '\t'+self._filename + '\n'
        container_str += '}\n'
        self._puml = container_str
...
class Enum(Container):
    ...
    """
    def __init__(self, header_enum, filename='', parent=None):
        """Constructor
...
class Namespace(list):
    ...
    def __init__(self, name, *args):
        ...
        """
        super().__init__(*args)
        self._container_type = 'namespace'
        self._name = name
        self._namespace = name
        self.__value__ = []
        if debug : print('namespace:',*args)
    ...
    def render(self):
        ...
        """
        if debug : print('namespace name',self._name)
        for c in self:
            if debug : print('c',c,c._namespace)
            self.__value__.append(c)
        if self._name:
...
class Diagram(object):
    ...
    """
    def __init__(self, template_file=None, flag_dep=False, flag_json=False):
        """Constructor
    ...
    def render(self):
        ...
        if debug : print('ns_list_in:',ns_list_in)
        ns_list = []
        ...
        for o in objects_out:
            if debug : print('oo',o._name,o)
        render = template.render(objects=objects_out,
        ...
        jjson = { 'objects': [] , 'inherit':[] , 'aggregation':[] , 'dependency':[] , 'nesting':[] }
        if self._flag_json:
            for obj in objects_out:
                jjson['objects'].append(obj.__dict__)
            for obj in self._inheritance_list:
                jjson['inherit'].append(obj.__dict__)
            for obj in self._aggregation_list:
                jjson['aggregation'].append(obj.__dict__)
            for obj in self._dependency_list:
                jjson['dependency'].append(obj.__dict__)
...
def CreatePlantUMLFile(file_list, output_file=None, output_json_file=None, **diagram_kwargs):
    ...
    output_json_file : str
        Name of the output json file including parsed information
    diagram_kwargs : dict
        Additional parameters passed to :class:`Diagram` constructor
    """
    #print('diagram_kwargs:',diagram_kwargs)
    ...
    diag_render , diag_json = diag.render()
    ...
    for tt in diag_json['objects']:
        if debug : print('type:',type(tt))
        if tt['_container_type'] == 'namespace':
            for vv in tt.get('__value__',[]):
                if isinstance(vv,dict):
                    if debug : print('--dict',vv)
                elif isinstance(vv,list):
                    if debug : print('--list',vv)
                    for l4 in vv:
                        if debug : print('l4',l4.__dict__)
                elif isinstance(vv,(ClassMethod,ClassVariable,EnumValue,Class,Namespace)):
                    if debug : print('--Class',vv.__dict__)
                else:
                    if debug : print('--else:',vv.__dict__)
            if debug : print('tt',tt)
    ...
    shutil.rmtree(PLANTUML_DIRECTORY,ignore_errors=True)
    os.makedirs(PLANTUML_DIRECTORY,exist_ok=True)
    create_plantuml_from_diag_json(diag_json)

def create_plantuml_from_diag_json(dj):
    create_plantuml_file(dj=dj,focus_class='',puml_kind='_puml',exception='')
    create_plantuml_file(dj=dj,focus_class='',puml_kind='_puml_nofile',exception='')
    create_plantuml_file(dj=dj,focus_class='',puml_kind='_puml_nomember',exception='')
    create_plantuml_file(dj=dj,focus_class='',puml_kind='_puml',exception='inherit_unknown')
    create_plantuml_file(dj=dj,focus_class='',puml_kind='_puml_nofile',exception='inherit_unknown')
    create_plantuml_file(dj=dj,focus_class='',puml_kind='_puml_nomember',exception='inherit_unknown')
    for o in dj['objects']:
         #create_plantuml_file(dj=dj,focus_class=o['_name'].split('::')[-1],puml_kind='_puml',exception='')
        create_plantuml_file(dj=dj,focus_class=o['_name'],puml_kind='_puml',exception='')
        create_plantuml_file(dj=dj,focus_class=o['_name'],puml_kind='_puml_nofile',exception='')
        create_plantuml_file(dj=dj,focus_class=o['_name'],puml_kind='_puml_nomember',exception='')
        create_plantuml_file(dj=dj,focus_class=o['_name'],puml_kind='_puml',exception='inherit_unknown')
        create_plantuml_file(dj=dj,focus_class=o['_name'],puml_kind='_puml_nofile',exception='inherit_unknown')
        create_plantuml_file(dj=dj,focus_class=o['_name'],puml_kind='_puml_nomember',exception='inherit_unknown')
    return

def create_plantuml_file(dj = [], focus_class='',puml_kind='_puml_nomember',exception='inherit_unknown'):
    if focus_class :
        file = focus_class + puml_kind
    else:
        file = '_all' + puml_kind
    if exception:
        file += '_exception_' + exception
    puml = ''
    puml += '@startuml\n'
    if not dj:
        puml += 'class Nothing\n'
        puml += '@enduml\n'
        return puml
    focus_related_class = set()
    if focus_class:
        focus_related_class.add(focus_class)
    class_name = []
    class_name_index = {}
    for k in dj:
        if k == 'objects':
            for i,o in enumerate(dj[k]):
                class_name_index[o['_name']] = i
                class_name.append(o['_name'])
    for k in dj:
        if not focus_class:
            break
        if k == exception:
            continue
        if k != 'objects': # relationship
            for o in dj[k]:
                if focus_class == o['_child'] or focus_class == o['_parent']:
                    focus_related_class.add(o['_child'])
                    focus_related_class.add(o['_parent'])
    ...
    for k in dj:
        ...
        if k == 'objects':
            for o in dj[k]:
                if focus_class:
                    if o['_name'] in focus_related_class:
                        puml += o[puml_kind]
                else:
                    #puml += wrap_namespace(o[puml_kind],o['_namespace'])
                    puml += o[puml_kind]
        if k == exception:
            continue
        if k != 'objects': # relationship
            for o in dj[k]:
                if focus_class:
                    if focus_class == o['_child'] or focus_class == o['_parent']:
                        if o['_child'] not in focus_related_class:
                            puml += 'class ' + o['_child'] + '\n'
                        if o['_parent'] not in focus_related_class:
                            puml += 'class ' + o['_parent'] + '\n'
                        puml += o['_parent'] + ' ' + LINK_TYPE_MAP[o['_link_type']] + ' ' + o['_child']  + '\n'
                else:
                    if o['_child'] not in focus_related_class:
                        puml += 'class ' + o['_child'] + '\n'
                    if o['_parent'] not in focus_related_class:
...
def make_inheritance_unknown(dj):
    dj['inherit_unknown'] = []
    inherit_pair = []
    for p in dj['inherit']:
         #child = p['_child_namespace'] + '::' + p['_child'] if p['_child_namespace'] else p['_child']
         #parent = p['_parent_namespace'] + '::' + p['_parent'] if p['_parent_namespace'] else p['_parent']
    ...
    for o in dj['objects']:
        ...
        if isinstance(o,dict):
            print(o)
            print(o['_name'])
            print(o['_puml_nomember'])
            print(o['_filename'])
            print('inheritance_list:',o['_inheritance_list'])
            print('inheritance_list:',getattr(o,'_inheritance_list',None))
        else:
            print(o._name)
            print(o._puml_nomember)
            print(o._inheritance_list)
        '''
        child = o['_name'].split('::')[-1]
        for inheritance in o['_inheritance_list']:
             #child = o['_namespace'] + '::' + o['_name'] if o['_namespace'] else o['_name']
             #print('..' , inheritance)
            ...
            if [child,parent] not in inherit_pair:
                ...
                dj['inherit_unknown'].append( {
                        ...
                        '_parent_namespace' : '',
                        '_child_namespace' : '',
                        '_puml' : inheritance + ' <|.. ' + o['_name'] + '\n'
                        } )
    return

def traverse_objects(boxList,obj,depth):
    if isinstance(obj,dict):
        if debug : print(depth,'dict', 'type',obj['_container_type'],  'name', obj['_name'],'namespace',obj['_namespace'])
        if obj['_container_type'] != 'namespace':
            boxList.append(obj)
        if obj.get('__value__',[]):
            if debug : print('    ','__value__')
            for _vv in obj.get('__value__',[]):
                traverse_objects(boxList,_vv,depth+1)
    elif isinstance(obj,(list,tuple)):
        if debug : print(depth,'list')
        for _vv in obj:
            traverse_objects(boxList,_vv,depth+1)
    elif isinstance(obj,(ClassMethod,ClassVariable,EnumValue,Class,Enum,Namespace)):
        if debug : print(depth,'Class', obj.__dict__.get('_container_type','^^'))
        if debug : print(depth,'Class', obj.__dict__.get('_name','^^'))
        if debug : print(depth,'Class', obj.__dict__)
        boxList.append(obj)
    else :
        if debug : print(depth,'else', obj.__dict__.get('_container_type','**'))
        if debug : print(depth,'else', obj.__dict__)
        boxList.append(obj)
...
def main():
    ...
    parser.add_argument('-o', '--output-file', dest='output_file',
                        required=False, default='', metavar='FILE',
                        help='output plantuml file')
    parser.add_argument('-j', '--output-json-file', dest='output_json_file',
                        required=False, default=None, metavar='FILE',
                        help='output json file including parsed information')
    parser.add_argument('-d', '--enable-dependency', dest='flag_dep',
                        required=False, default=False, action='store_true',
                        help='Extract dependency relationships from method ' +
                        'arguments')
    parser.add_argument('-f', '--focus-class', dest='focus_class',
                        required=False, default='', metavar='string-with-comma',
                        help='focus class to show different color')
    ...
    if len(args.input_files) > 0:
        ...
        if args.focus_class.strip():
            FOCUS_CLASS = args.focus_class.strip().split(',')
        CreatePlantUMLFile(args.input_files, args.output_file,args.output_json_file,
