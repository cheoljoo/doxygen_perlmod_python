...
ifndef OUTDIR
	...
	if [ ! -e plantuml.json ]; then echo "taf_run_flowchart_in_dockerTTT.sh" ;  mkdir -p ${OUTDIR};  docker run -it  --user `id -u`:`id -g`  -v `pwd`/_hpp_:/docker_in -v `realpath ${OUTDIR}`:/docker_out -v `pwd`:/docker_git -e DOCKER_IN=/docker_in -e DOCKER_OUT=/docker_out -e DOCKER_GIT=/docker_git -e DOCKER_MODULE=${MODULE} doxygen_perlmod_python:latest  bash /docker_git/taf_run_in_docker.sh ; fi
