...
class Dog {
        ...
        #checkEnergy(int energyToExpend) : bool
        -name : const char*
        -energy : int
        +bark() : void
        +run(int minutes) : void
        +sleep(int minutes) : void
}
@enduml
...
class Dog {
  public:
     ...
     * minute.
     *
     * \param [in] minutes Number of minutes for the dog to run.
     */
